...
import { selectTabByShortcut, useTabSelectionShortcut } from "./use-tab-selection-shortcut";
...
function stubDesktopAPI(kind: "main" | "issue") {
  ...
  const onSelectTabShortcut = vi.fn((callback: (key: TabSelectionShortcutKey) => void) => {
    handler = callback;
    return unsubscribe;
  });
  (window as unknown as { desktopAPI: Record<string, unknown> }).desktopAPI = {
    windowContext: kind === "main" ? { kind: "main" } : { kind: "issue", path: "/acme/issues/abc", workspaceSlug: "acme" },
    onSelectTabShortcut,
  };
  return { onSelectTabShortcut, unsubscribe, deliver: (key: TabSelectionShortcutKey) => handler?.(key) };
}
...
describe("selectTabByShortcut", () => {
  ...
  it.each([1, 2, 3, 4, 5, 6, 7, 8] as const)("selects exact tab position %i", (key) => {
    selectTabByShortcut(key);
    expect(activeTabId()).toBe(`t${key}`);
  });

  it.each([1, 3, 10])("selects the last of %i tabs for shortcut 9", (tabCount) => {
    seedTabs(tabCount);
