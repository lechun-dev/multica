      ...
      entries: [
        {
          version: "0.5.2",
          date: "2026-09-23",
          title: "デプロイの信頼性と Agent の協調体験を改善",
          changes: [],
          features: [],
          improvements: [
            "サービス公開前にデータベース移行を自動実行し、移行に失敗した場合はデプロイを停止するようになりました。",
            "ワークスペースのランタイムモデルを daemon の検出結果と統合・重複排除し、設定済みモデルを常に選択できるようにしました。",
            "Agent の実行サマリー、タスク履歴、通知による再開体験を改善しました。",
          ],
          fixes: [
            "ステータスアイコン列のスキーマ差異によってタスク状態一覧の読み込みに失敗する問題を修正しました。",
            "リリースビルドの依存関係と安定版の変更履歴検証タイミングを修正しました。",
          ],
        },
        {
          version: "0.4.89",
        ...
        },
        {
          ...
          changes: [],
          features: [
            "タスクのタイムラインで Agent の推論進行をプレビューし、コメントやタスク説明に注釈を付けられるようになりました。",
          ],
          improvements: [
            "受信トレイがタスクの変更をすぐに反映し、コメント通知を簡潔なプレビューで表示します。",
            "自動再試行で元の作業ディレクトリを維持し、デスクトップ版に便利な戻る・進む履歴メニューを追加しました。",
          ],
          fixes: [
            "最新の上流版に合わせてマイグレーションとサーバー統合を調整し、MissionOS 独自の権限、通知、チャット機能を維持しました。",
            "デスクトップ版のタスク環境で `multica` と `missionos` の両方を使えるようにし、開始直後に CLI を探し回らないようにしました。",
          ],
        },
        {
          version: "0.4.82",
