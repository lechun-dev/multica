...
// 2026-09-22 coder(lq): Authenticate all runtimes in one query and seed the
// connection-scoped heartbeat leases to avoid repeated database reads.
func (h *Handler) buildDaemonWebSocketIdentity(w http.ResponseWriter, r *http.Request, runtimeIDs []string, userID string) (daemonws.ClientIdentity, bool) {
	...
	if len(workspaceIDs) > 0 {
		identity.WorkspaceID = workspaceIDs[0]
	}
