...
// 2026-09-12 coder(lq): Restore upstream task-run scope coverage after merging it
// with the private migration history; the referenced migration is renumbered below.
...
func TestCancelCommentAssigneeFallbacksMigration(t *testing.T) {
	...
	sql, err := os.ReadFile("../../migrations/497_cancel_comment_assignee_fallbacks.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	for range 2 {
