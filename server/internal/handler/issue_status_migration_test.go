...
func TestIssueStatusLifecycleMigrationPreservesIdentity(t *testing.T) {
	...
	migration, err := os.ReadFile("../../migrations/536_issue_status_lifecycle_categories.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	iconMigration, err := os.ReadFile("../../migrations/537_issue_status_icon.up.sql")
	if err != nil {
		t.Fatal(err)
	}
