...
| Status change (incl. → `cancelled`) does NOT cancel in-flight tasks; issue deletion and explicit issue archive do | no-cancel note in `server/internal/handler/issue.go:2652-2658` (`UpdateIssue`) and `:3170-3171` (`BatchUpdateIssues`); deletion still cancels at `:2863` (`DeleteIssue`) / `:3239` (`BatchDeleteIssues`), while archive cleanup runs at `server/internal/handler/issue_archive.go:79` via `CancelTasksForIssue` (`server/internal/service/task.go:2560`) | new citation |
