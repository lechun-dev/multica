...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// 2026-09-12 coder(lq): Desktop ships the CLI as missionos, but task
	// prompts still tell agents to run `multica`. Stage both names on PATH
	// so agents resolve the CLI immediately and do not search the disk.
	injectTaskCLIPATH(agentEnv, taskTempDir, taskLog)
