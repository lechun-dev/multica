...
// 2026-09-12 coder(lq): Keep upstream migration coverage aligned with the private-history renumbering.
// issuePropertiesBigramMigrations is the pair that has to move together: 487
// builds the expression index, 488 generates the statistics the planner needs
// before it will consider it. Applying 487 alone leaves the index built, valid
// and ignored.
var issuePropertiesBigramMigrations = []string{
	"487_issue_properties_bigm_index",
	"488_issue_properties_bigm_index_statistics",
}

// TestIssuePropertiesBigramIndexBuildsOnlyWherePGBigmExists runs migrations 487
// and 488's real SQL through the runner. The gate has to hold in both
...
// unaccelerated.
//
// 488 is not gated, so its statistics refresh must land in both environments.
func TestIssuePropertiesBigramIndexBuildsOnlyWherePGBigmExists(t *testing.T) {
	...
	// 488 ran unconditionally, so the table's own column statistics exist in
	// every environment.
	if got := statisticsRowCount(t, ctx, pool, schema, "issue"); got == 0 {
		t.Fatal("488 did not analyze the table: no statistics rows")
	}
	...
	// where the up direction never built anything. 488's rollback is a
...
// 488 exists for, and it can only be written against the real extension.
//
...
// estimate is the thing 488 repairs. Which plan that estimate wins is a
...
func TestIssuePropertiesBigramIndexNeedsAnalyzeForItsExpression(t *testing.T) {
	...
	applyMigration(t, "487_issue_properties_bigm_index")
	assertIndexValidity(t, pool, schema, "idx_issue_properties_bigm", true)
	...
	if withoutStatistics < actual*10 {
		t.Fatalf("expected the index build alone to leave a badly wrong estimate, got %d for %d actual rows; "+
			"if Postgres now collects expression statistics at build time, 488 and this test can go",
			withoutStatistics, actual)
	}

	applyMigration(t, "488_issue_properties_bigm_index_statistics")
	if got := statisticsRowCount(t, ctx, pool, schema, "idx_issue_properties_bigm"); got == 0 {
		t.Fatal("488 generated no statistics for the LOWER(properties::text) expression")
	}
	withStatistics := estimatedRows(t, ctx, pool, prefilterQuery)
	if withStatistics*10 > withoutStatistics {
		t.Fatalf("488 did not improve the selectivity estimate: %d before, %d after, %d actual",
