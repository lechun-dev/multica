...
USAGE
  missionos <command> <subcommand> [flags]
{{range .Groups}}
{{.Title}}
...
EXAMPLES
  $ missionos login
  $ missionos issue list --output json
  $ missionos daemon start
  $ missionos agent list --output json
...
LEARN MORE
  Use ` + "`missionos <command> <subcommand> --help`" + ` for more information about a command.
`
