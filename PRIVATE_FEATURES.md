...
| Private release notes | Private production and preview releases publish the MissionOS changelog rather than the official Multica changelog. Production Release notes include every non-merge commit since the previous stable tag and are refreshed identically by both publishers. | `apps/web`, desktop changelog link, private release workflows, `scripts/generate-release-notes.mjs` | release changelog and generated-note policy tests |
