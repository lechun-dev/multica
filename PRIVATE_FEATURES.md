...
| Deployment release channels | Production accepts only stable image tags; staging accepts stable and beta tags; test accepts stable, beta, and test tags. Every deployment uses an explicit immutable tag and validates it before connecting to the host. | `.github/workflows/deploy*.yml`, `scripts/validate-deploy-tag.mjs` | `validate-deploy-tag.test.mjs` |
