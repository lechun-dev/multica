...
if [ "$mode" = "build" ]; then
  ...
  echo "Images: ${MULTICA_BACKEND_IMAGE:-ghcr.io/lechun-dev/multica-backend}:${MULTICA_IMAGE_TAG:-latest}"
  echo "        ${MULTICA_WEB_IMAGE:-ghcr.io/lechun-dev/multica-web}:${MULTICA_IMAGE_TAG:-latest}"
fi
echo ""
