...
export function IssuePermissionsPanel({ issueId, projectId }: { issueId: string; projectId?: string | null }) {
  ...
  return <div className="rounded-lg border p-3 space-y-3"><div className="flex items-center gap-2 text-sm font-medium"><Shield className="size-4" />任务权限（需先拥有项目查看权限）</div><div className="flex gap-2"><Select value={userId} onValueChange={(v) => setUserId(v ?? "")}><SelectTrigger className="flex-1"><SelectValue placeholder="选择成员" /></SelectTrigger><SelectContent>{members.filter((m) => !granted.has(`${m.user_id}:${permission}`)).map((m) => <SelectItem key={m.user_id} value={m.user_id}>{m.name}</SelectItem>)}</SelectContent></Select><Select value={permission} onValueChange={(v) => setPermission(v ?? "project.edit")}><SelectTrigger className="w-36"><SelectValue /></SelectTrigger><SelectContent>{["project.edit", "project.issue.manage", "project.agent.use"].map((p) => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select><Button size="icon" onClick={add} disabled={!userId}><UserPlus className="size-4" /></Button></div>{grants.map((g) => <div key={`${g.user_id}:${g.permission}`} className="flex items-center gap-2 text-sm"><span className="flex-1">{members.find((m) => m.user_id === g.user_id)?.name ?? g.user_id}</span><span className="text-xs text-muted-foreground">{g.permission}</span><Button variant="ghost" size="icon" onClick={() => remove(g)}><UserMinus className="size-4" /></Button></div>)}</div>;
}
