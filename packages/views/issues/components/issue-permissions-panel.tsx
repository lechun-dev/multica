...
export function IssuePermissionsPanel({ issueId, projectId }: { issueId: string; projectId?: string | null }) {
  ...
  const add = async () => { if (!userId) return; try { await api.grantIssuePermission(issueId, { user_id: userId, permission }); setUserId(""); refresh(); toast.success(t(($) => $.permissions.grant_success)); } catch (e) { toast.error(e instanceof Error ? e.message : t(($) => $.permissions.grant_failed)); } };
  const remove = async (g: { user_id: string; permission: string }) => { try { await api.revokeIssuePermission(issueId, g.user_id, g.permission); refresh(); toast.success(t(($) => $.permissions.revoke_success)); } catch (e) { toast.error(e instanceof Error ? e.message : t(($) => $.permissions.revoke_failed)); } };
  return <div className="rounded-lg border p-3 space-y-3"><div className="flex items-center gap-2 text-body font-medium"><Shield className="size-4" />{t(($) => $.permissions.title)}</div><div className="flex gap-2"><Select items={memberItems} value={userId} onValueChange={(v) => setUserId(v ?? "")}><SelectTrigger className="flex-1"><SelectValue placeholder={t(($) => $.permissions.select_member)} /></SelectTrigger><SelectContent>{members.filter((m) => !granted.has(`${m.user_id}:${permission}`)).map((m) => <SelectItem key={m.user_id} value={m.user_id}>{m.name}</SelectItem>)}</SelectContent></Select><Select items={permissionItems} value={permission} onValueChange={(v) => setPermission(v ?? "project.edit")}><SelectTrigger className="w-36"><SelectValue /></SelectTrigger><SelectContent>{permissionValues.map((p) => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select><Button size="icon" onClick={add} disabled={!userId}><UserPlus className="size-4" /></Button></div>{grants.map((g) => <div key={`${g.user_id}:${g.permission}`} className="flex items-center gap-2 text-body"><span className="flex-1">{members.find((m) => m.user_id === g.user_id)?.name ?? g.user_id}</span><span className="text-caption text-muted-foreground">{g.permission}</span><Button variant="ghost" size="icon" onClick={() => remove(g)}><UserMinus className="size-4" /></Button></div>)}</div>;
}
