...
vi.mock("@tanstack/react-query", async (importOriginal) => ({
  ...
  useQuery: (options: { queryKey: readonly unknown[]; enabled?: boolean }) => {
    ...
    const isLookup = options.queryKey.includes("lookup");
    return {
      data: isLookup
        ? {
            items: listData.lookup ?? listData.archived,
            hasMore: false,
            nextCursor: null,
          }
        : listData.active,
      isLoading: isLookup ? lookupState.isLoading : false,
      isError: isLookup ? lookupState.isError : false,
      refetch: isLookup ? lookupState.refetch : vi.fn(),
    };
  },
