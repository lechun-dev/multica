...
describe("useActorName", () => {
  ...
  it("returns a referentially stable getActorName across renders during cold load", () => {
    ...
    expect(result.current.hasActor("agent", "agent-1")).toBeUndefined();
  });
  ...
  it("resolves names once the directories are loaded", () => {
    ...
    expect(result.current.hasActor("agent", "agent-1")).toBe(true);
    expect(result.current.hasActor("agent", "private-agent")).toBe(false);
  });
});
