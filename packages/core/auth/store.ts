...
export interface AuthState {
  ...
  loginWithDingTalk: (code: string, state: string) => Promise<User>;
...
export function createAuthStore(options: AuthStoreOptions) {
  ...
  return create<AuthState>((set, get) => ({
    ...
    loginWithDingTalk: async (code: string, state: string) => {
      const { token, user } = await api.dingTalkLogin(code, state);
      if (!cookieAuth) {
        storage.setItem("multica_token", token);
        api.setToken(token);
      }
